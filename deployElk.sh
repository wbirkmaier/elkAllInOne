...
sed 's/# server.host: "0.0.0.0"/server.host: "localhost"/g' /opt/kibana/config/kibana.yml.bak > /opt/kibana/config/kibana.yml
...
cat <<'EOF' >> /etc/logstash/conf.d/05-syslog-input.conf
input {
   tcp {
     port => 5000
     type => "syslogd"
...
cat <<'EOF' >> /etc/logstash/conf.d/11-syslog-filter.conf
filter {
  if [type] == "syslogd" {
    grok {
      match => { "message" => "%{SYSLOGTIMESTAMP:syslog_timestamp} %{SYSLOGHOST:syslog_hostname} %{DATA:syslog_program}(?:\[%{POSINT:syslog_pid}\])?: %{GREEDYDATA:syslog_message}" }
